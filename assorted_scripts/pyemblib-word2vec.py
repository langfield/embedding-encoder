    ...
    for line_index,line in enumerate(hook):
        if len(line.strip()) > 0:
            ...
            try:
                ...
                while True:
                    try:
                        word, vector = "".join(chunks[0:i]).strip(), np.array([float(n) for n in chunks[i:]])
                    except Exception as e:
                        i += 1
                        if i == len(chunks) - 1:
                            raise e
            ...
            if len(vector) == dim - 1:
                ...
                print("<<< O NO >>>")
                print("Problem key: ", word)
                print("Problem vector: ", vector)
                print("Problem at: ", line_index, len(word))
                #raise ValueError("Read %d-length vector, expected %d" % (len(vector), dim))
                print("Read %d-length vector, expected %d" % (len(vector), dim))
                print("Skipping.")
