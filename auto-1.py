...
for i in range(1,101):
    intList.append(i)
embWithIndex[intList] = pd.DataFrame(embWithIndex.wordsWithFriends.values.tolist(), index=embWithIndex.index)
realValEmbs = embWithIndex[intList]
print(realValEmbs)
...
#this is the number of rows in the dataset, i.e. the number of unique words in the embedding
numInputs = shape[0]
...
# -ors. 
def nextBatch(xData,batchSize,iteration):
    ...
    for i in range(batchSize):
        ...
        distRow = tf.tensordot(tf.slice(xData,sliceBegin,batchSliceSize),xData,[[1],[1]]) # dot product
        print("distRow shape is: ",distRow.shape)
        distRowList.append(distRow[0]) 
        currentIndex = currentIndex + 1
    # print("distRowList shape is: ",distRowList.shape)
    distMatrix = tf.stack(distRowList)    
...
batchSize = 2
iteration = 0
XBatch = nextBatch(dataTensor,batchSize,iteration)
